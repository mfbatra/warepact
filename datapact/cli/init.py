"""datapact init — scaffold a contracts/ directory."""
...
from datapact.cli._credentials import detect_warehouse_from_env, get_required_env_vars
...
def init_command(
    ...
    if required_vars:
        console.print(
            ...
            "Set these env vars before running [bold]datapact check[/]:\n"
            f"  {', '.join(required_vars)}"
        )

    console.print(
        f"\n[bold green]Done![/] Edit [cyan]{example_path}[/] "
        f"then run [bold]datapact check {name}[/]"
    )
