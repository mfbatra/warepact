...
if TYPE_CHECKING:
    ...
    from warepact.interfaces.validator import Validator
...
class PluginRegistry:
    ...
    def autodiscover(cls) -> None:
        """
        Import every module under warepact/adapters/ and warepact/plugins/.

        Importing a module that contains @register_* decorators is enough to
        trigger registration — no explicit calls needed.
        """
        for package_name in ("warepact.adapters.warehouses", "warepact.adapters.alerting"):
            cls._import_package(package_name)

        # Community plugins installed as warepact.plugins.*
        cls._import_package("warepact.plugins")
